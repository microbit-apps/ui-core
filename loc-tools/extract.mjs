...
//
// Scans the .ts files listed in a project's pxt.json "files" (so test files are
// excluded) and extracts the source display strings passed to the localization
// helpers:
//
//   ui.loc("...")  / loc("...")
...
// convention; see a repo's locales/README.md). Any other argument is reported
// as a loud warning naming the file and the argument, so a missed string is
// never dropped silently. A call whose argument is deliberately dynamic (its
// strings reach the catalog by another route) is suppressed by putting
// "locstrings-ignore" in a comment on the call's line.
//
// The result is a sorted object mapping each catalog key to its source display
// string. For locc entries the value is the plain string, without the context
// prefix. The catalog is fully derived from the current sources.
...
const LOCC_RE = new RegExp(`${DEF}(?:ui\\.)?locc\\s*\\(\\s*(${ARG})\\s*,\\s*(${ARG})`, "g")
...
// Extract the source catalog from a project's listed files. Returns the sorted
// catalog and the count of unresolved (non-literal) helper arguments, each of
// which is reported to the console as it is found.
export function extractCatalog(root) {
    const pxt = JSON.parse(readFileSync(join(root, "pxt.json"), "utf8"))
    const catalog = {}
    let warnings = 0

    // Resolve a captured helper argument to its string value, or null (with a
    // warning) when it is not a string literal.
    const resolveArg = (raw, file) => {
        const arg = raw.trim()
        if (arg == "") return null
    ...
    return { catalog: sorted, warnings }
}

// Regenerate a project's source catalog file (locales/<srcLang>.json) from its
// listed files.
export function runLocStrings(root, srcLang = "en") {
    const { catalog, warnings } = extractCatalog(root)
    writeFileSync(join(root, "locales", srcLang + ".json"), JSON.stringify(catalog, null, 4) + "\n")
    const count = Object.keys(catalog).length
    console.log(`locstrings: wrote ${count} key(s) to locales/${srcLang}.json`)
